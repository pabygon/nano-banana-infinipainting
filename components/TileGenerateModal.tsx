...
export function TileGenerateModal({ open, onClose, x, y, z, onUpdate }: TileGenerateModalProps) {
  ...
  // Acquire generation lock when modal opens
  useEffect(() => {
    ...
    const acquireLock = async () => {
      try {
        ...
        if (response.status === 423) {
          ...
          setLockError(data.error || 'This tile is currently being worked on by another user');
          setHasLock(false);
          return;
        }
        ...
        if (response.ok) {
          ...
          throw new Error('Failed to acquire generation lock');
        }
      } catch (err) {
        console.error('Failed to acquire generation lock:', err);
        setLockError('Failed to acquire lock for tile generation');
        setHasLock(false);
      }
    };
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      if (response.status === 423) {
        const data = await response.json();
        setError(data.error || "Tile is currently being generated by another user");
        return;
      }
        ...
        >
          <div className="flex flex-col h-full" onClick={(e) => e.stopPropagation()}>
            <div className="px-4 pt-4">
              <Dialog.Title className="text-lg flex items-center gap-2">
                Generate Preview
                {lockError && (
                  <div className="flex items-center gap-1 px-2 py-1 bg-red-100 text-red-700 text-xs rounded-md">
                    🔒 Locked
                  </div>
                )}
              </Dialog.Title>
                ...
                <div className="relative">
                  ...
                  <div className="absolute bottom-2 right-2">
                    <button
                      ...
                      disabled={loading || !prompt.trim() || !clientId || !hasLock}
              ...
              <div className="px-0 pb-0" onClick={(e) => e.stopPropagation()}>
                <div className="flex w-full items-center justify-end gap-2">
                  ...
                  <button
                    ...
                    disabled={loading || !previewTiles || selectedPositions.size === 0 || !clientId || !hasLock}
